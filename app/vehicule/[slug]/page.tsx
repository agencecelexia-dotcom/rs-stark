...
export default function VehiclePage() {
  ...
  if (!vehicle) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 96 }}>
        <p className="font-display" style={{ fontSize: 64, color: 'rgba(0,0,0,0.08)', marginBottom: 16 }}>404</p>
        <p style={{ color: 'rgba(0,0,0,0.45)', marginBottom: 32 }}>Véhicule introuvable</p>
  ...
  const INPUT_STYLE: React.CSSProperties = {
    background: '#F8F9FA', border: '1px solid #2A2A2A', color: '#0F0F0F',
  ...
  return (
    <div className="page-section" style={{ minHeight: '100vh', paddingTop: 96, paddingBottom: 96 }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 11, fontFamily: 'var(--font-code,monospace)', color: 'rgba(0,0,0,0.35)' }}>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px' }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            <div className="img-ph" style={{ aspectRatio: '16/10', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
              <span className="font-display" style={{ fontSize: 52, color: 'rgba(0,0,0,0.04)', letterSpacing: '0.05em', userSelect: 'none' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.65)', border: '1px solid #2A2A2A', padding: 24 }}>
              <p style={{ fontSize: 10, fontFamily: 'var(--font-code,monospace)', letterSpacing: '0.3em', color: 'rgba(0,0,0,0.28)', textTransform: 'uppercase', marginBottom: 16 }}>Inclus avec ce véhicule</p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <h1 className="font-display" style={{ fontSize: 'clamp(40px,6vw,72px)', color: '#0F0F0F', lineHeight: 0.95, marginBottom: 8 }}>{vehicle.modele}</h1>
            <p style={{ fontSize: 18, color: 'rgba(0,0,0,0.45)', marginBottom: 24 }}>{vehicle.version} · {vehicle.annee}</p>
            ...
            <div style={{ paddingBottom: 24, borderBottom: '1px solid #2A2A2A', marginBottom: 24 }}>
              {vehicle.prix ? (
                ...
                <p style={{ fontFamily: 'var(--font-code,monospace)', fontSize: 24, color: 'rgba(0,0,0,0.35)' }}>Prix sur demande</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3" style={{ marginBottom: 24 }}>
              {specs.map((spec) => (
                <div key={spec.label} style={{ background: 'rgba(255,255,255,0.65)', border: '1px solid #2A2A2A', padding: 16 }}>
                  <spec.icon size={13} style={{ color: '#C9A84C', marginBottom: 8 }} />
                  <p style={{ fontSize: 10, color: 'rgba(0,0,0,0.35)', letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: 'var(--font-code,monospace)', marginBottom: 4 }}>{spec.label}</p>
                  <p style={{ fontFamily: 'var(--font-code,monospace)', fontSize: 13, color: '#0F0F0F' }}>{spec.value}</p>
                </div>
              ))}
            </div>
            ...
            {vehicle.statut === 'vente' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                <button
                  ...
                  style={{ padding: '16px', background: '#C9A84C', color: '#000', fontFamily: 'var(--font-heading,sans-serif)', fontSize: 15, letterSpacing: '0.2em', border: 'none', width: '100%',}}
                >
                  RÉSERVER CE VÉHICULE
                </button>
                <button
                  onClick={() => setShowModal('financement')}
                  style={{ padding: '16px', background: 'transparent', color: '#0F0F0F', fontFamily: 'var(--font-heading,sans-serif)', fontSize: 14, letterSpacing: '0.15em', border: '1px solid #2A2A2A', width: '100%',}}
                >
                  DEMANDER UN FINANCEMENT
                </button>
                <div className="grid grid-cols-2 gap-3">
                  <Link href="/reprise" style={{ padding: '12px', border: '1px solid #2A2A2A', color: 'rgba(0,0,0,0.45)', textAlign: 'center', fontSize: 12, fontFamily: 'var(--font-code,monospace)', textDecoration: 'none', display: 'block' }}>
        ...
        <div style={{ marginTop: 64 }}>
          <Link href="/vente" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: 'rgba(0,0,0,0.35)', fontSize: 13, fontFamily: 'var(--font-code,monospace)', textDecoration: 'none' }}>
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              style={{ background: 'rgba(255,255,255,0.65)', border: '1px solid #2A2A2A', padding: 32, maxWidth: 480, width: '100%' }}
            >
              {formSent ? (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <h3 className="font-display" style={{ fontSize: 28, color: '#0F0F0F', marginBottom: 12 }}>DEMANDE ENVOYÉE</h3>
                  <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)' }}>Notre équipe vous contacte dans les 2 heures ouvrées.</p>
                  <button onClick={() => { setShowModal(null); setFormSent(false) }} style={{ marginTop: 24, color: '#C9A84C', fontSize: 12, fontFamily: 'var(--font-code,monospace)', background: 'none', border: 'none',}}>Fermer</button>
                </div>
              ) : (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                    <h3 className="font-display" style={{ fontSize: 22, color: '#0F0F0F' }}>
                      {showModal === 'reserve' ? 'RÉSERVER CE VÉHICULE' : 'DEMANDER UN FINANCEMENT'}
                    </h3>
                    <button onClick={() => setShowModal(null)} style={{ color: 'rgba(0,0,0,0.35)', background: 'none', border: 'none', fontSize: 18, lineHeight: 1,}}>✕</button>
                  </div>
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); setFormSent(true) }} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <button type="submit" style={{ padding: '16px', background: '#C9A84C', color: '#000', fontFamily: 'var(--font-heading,sans-serif)', fontSize: 14, letterSpacing: '0.2em', border: 'none', marginTop: 8,}}>
